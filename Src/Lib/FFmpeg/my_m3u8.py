...
class M3U8_Segments:
    ...
    def get_info(self):
        ...
        custom_headers['user-agent'] = get_headers()
        response = requests.get(self.url, headers=custom_headers)
    ...
    def get_req_ts(self, ts_url):
        ...
        is_valid = ( url_number not in failed_segments ) 
    ...
    if m3u8_playlist != None:
        ...
        if m3u8_index == None:
            ...
            if "https" in m3u8_index:
                if log: 
                    console.log(f"[green]Select m3u8 index => [purple]{m3u8_index}")
            else:
